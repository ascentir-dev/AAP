...
<head>
  ...
  <meta property="og:description" content="Frank from Ascentir recorded this specifically for you." />
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    h1 {{ font-size: 1.4rem; margin-bottom: 16px; font-weight: 500; color: #e0e0e0; }}
    video {{ width: 100%; border-radius: 12px; box-shadow: 0 8px 40px rgba(0,0,0,0.6); }}
...
<body>
  <div class="container">
    <h1>Hey {first_name} — Frank recorded this for you</h1>
    <video autoplay muted controls playsinline>
      <source src="{video_url}" type="video/mp4">
    </video>
    <div class="cta">
      <a href="{book_a_call_url}" target="_blank" rel="noopener">Book A Call &rarr;</a>
    </div>
  </div>
</body>
</html>
...
async def generate_landing_page(
    lead: dict[str, Any], video_url: str, settings: Any
) -> str:
    """Render HTML landing page and upload to R2. Returns the Cloudflare Pages URL."""
    html = _LANDING_PAGE_TEMPLATE.format(
        first_name=lead.get("first_name", ""),
        video_url=video_url,
        book_a_call_url=settings.book_a_call_url,
    )
